...
bool Wall::m_registerit = Factory<StaticObject>::registerIt(WallObject, [](const sf::Vector2f& position, b2World* world) -> std::unique_ptr<StaticObject> { return std::make_unique<Wall>(position, world); });





Wall::Wall(const sf::Vector2f& position_vec, b2World* world) : StaticObject(Resources::instance().getTexture(WallTexture),
